...
class Cache
{
    ...
    {
        return System::getCurrentMemoryUsage(false) - CACHE_START_MEMORY;
    }
    ...
    public static function load(string $path, string $savedTag) : void
    {
        ...
        $error = function($message) use ($path) {
            return new FileNotFoundException('Could not load file \'' . $path . '\': ' . $message);
        };
    ...
    final public static function create(string $savedTag, array $data = []) : object
    {
        ...
        if(!defined('CACHE_START_MEMORY')) {
            define('CACHE_START_MEMORY', System::getCurrentMemoryUsage(false));
        }
