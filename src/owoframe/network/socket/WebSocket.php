...
class WebSocket
{
    ...
    public function __construct(string $ip = '0.0.0.0', int $port = 32710, bool $autoCreate = true)
    {
        ...
        if($autoCreate) {
            ...
            if(!$this->socket) {
                $this->logger->emergency('Failed to create socket!');
                return;
            }
            socket_set_option($this->socket, SOL_SOCKET, SO_REUSEADDR, 1);
            socket_bind($this->socket, $this->ip, $this->port);
            if(!socket_listen($this->socket, $this->port)) {
                $this->logger->error("Could not bind to address '{$this->getAddress()}'! Error message: " . socket_strerror(socket_last_error($this->socket)));
                return;
            }
            $this->created = true;
            $this->logger->success('OwO WebSocket Server is listening on §3' . $this->toString());
        }
    }
    ...
    public function run(?callable $callback = null, ?array $args = null) : void
    {
        static $isRunning;
        if(!$this->created) {
            $this->logger->error("Invalid Socket! Error message: " . socket_strerror(socket_last_error()));
            return;
        }
        if($isRunning) {
            $this->logger->error('Do not call twice this method when WebSocket Server is running!');
            return;
        }
        if(!isset($isRunning)) {
            $clients[] = $this->socket;
            ...
            foreach($changes as $ip => $client) {
                ...
                if($this->socket == $client) {
                    if(!($newClient = socket_accept($client))) {
                        $this->logger->error('Failed to accept socket: ' . socket_strerror(socket_last_error($client)));
                        continue;
                    }
                    $this->handshaking($newClient, trim(socket_read($newClient, 1024)), $data);
                    socket_getpeername($newClient, $ip);
                    $clients[$ip] = $newClient;
                    $this->logger->notice('New Client connected! IP: ' . $ip);
                    $this->logger->info('User Agent: ' . $data['User-Agent']);
                } else {
                    if(@!socket_recv($client, $data, 2048, 0)) {
                        continue;
                    }
                    $this->decodeData($data);
                    $this->logger->info("Received data from Client[{$ip}]: " . $data);
                    $response = 'Hello World! Welcome to use OwOFrame Easy Websocket Widget!';
                    if($_ = call_user_func_array($callback, $args)) {
                        $response = $_;
                    }
                    $this->send($client, $response);
                    $this->logger->debug('Response to Client with: ' . $response);
                }
            }
        }
    }
    ...
    public function handshaking($newClient, string $line, &$clientData = [])
    {
        ...
        if(!isset($clientData['Sec-WebSocket-Key'])) {
            ...
            $this->logger->error("Could not make HandShake to address '{$this->getAddress()}'! Error message: {$errorMsg}");
