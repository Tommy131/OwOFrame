...
class Path
{
		...
		if(preg_match_all($regex[0], $template, $matches))
		{
			foreach($matches[0] as $k => $tag) {
				...
				if(preg_match($regex[1], $tag, $m)) {
					...
					if(!$status) {
						$strings[] = Helper::findTagNewline($tag, $template);
						$replace[] = '';
					} else {
	...
	final public static function getViewPath(string $index, &$isExists = false) : string
	{
		...
		$path     = HttpManager::getCurrent('app')::getAppPath() . 'view' . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
	...
	{
		self::filterPath($index);
		$path     = self::getViewPath('component') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getViewPath('static') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getStaticPath('css') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getStaticPath('js') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getStaticPath('img') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = RESOURCE_PATH . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getResourcePath('img') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getResourcePath('js') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
	...
	{
		self::filterPath($index);
		$path     = self::getResourcePath('css') . DIRECTORY_SEPARATOR . Helper::escapeSlash($index);
		$isExists = self::exists($path);
		return $path;
	}
