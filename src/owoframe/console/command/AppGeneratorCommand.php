...
class AppGeneratorCommand extends \owoframe\console\CommandBase
{
	public function execute(array $params) : bool
	{
		...
		if(empty($appName)) {
			LogWriter::info('Please enter a valid appName. Usage: ' . self::getUsage('newapp [string:appName]'));
			return false;
		}
		...
		if(is_dir($appPath)) {
			LogWriter::info("Application '{$appName}' may exists, please delete/rename/move it and then use this command.");
			return false;
		} else {
			...
			if(AppManager::hasApp($appName)) {
				LogWriter::success("Generated empty AppFrame '{$upAppName}' successfully. Please check the app path '{$appPath}' to develop it.");
				return true;
			} else {
				LogWriter::error('An unknown error caused that cannot generate this empty AppFrame!');
				return false;
			}
		}
	}
