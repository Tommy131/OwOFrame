...
class AppGeneratorCommand extends \owoframe\console\CommandBase
{
	public function execute(array $params) : bool
	{
		...
		if(is_dir($appPath)) {
			Logger::info("Application '{$appName}' may exists, please delete/rename/move it and then use this command.");
			return false;
		} else {
			...
			if(AppManager::hasApp($appName)) {
				Logger::success("Generated empty AppFrame '{$upAppName}' successfully. Please check the app path '{$appPath}' to develop it.");
				return true;
			} else {
				Logger::error('An unknown error caused that cannot generate this empty AppFrame!');
				return false;
			}
		}
	}
