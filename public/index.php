...
$master  = new MasterManager($classLoader);
$http    = $master->getUnit('http');
$logger  = $master->getUnit('logger');
...
if($mode === 'static.owo')
{
	if(count($parser) === 2) {
		...
		if(is_file($file)) {
			...
			if(is_array($tempData)) {
				if(isset($tempData['expireTime'])) {
					if(microtime(true) - $tempData['expireTime'] <= 0) {
						...
						$logger->info('[403@Access Denied=' . $reason . '] ' . Helper::getClientIp() . ' -> ' . HttpManager::getCompleteUrl());
						stdDie($reason, '', 403);
					}
				}
			...
			$logger->info('[404@Not Found] ' . Helper::getClientIp() . ' -> ' . HttpManager::getCompleteUrl());
			stdDie('REQUESTED FILE NOT FOUND', '<p>FILE: ' . $hashTag . '.' . $type . '</p>');
		}
	} else {
		$logger->info('[403@Access Denied] ' . Helper::getClientIp() . ' -> ' . HttpManager::getCompleteUrl());
		stdDie('', '', 403);
	}
} else {
	$http->start();
